...
import { categories, categoryFilters, mergeSeedThreads, threads, type Category } from "@/lib/data";
...
export default async function CategoriesPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  const activeFilter = categoryFilters.find((filter) =>
    filter.slug === requestedCategory || (filter.categories as readonly Category[]).includes(requestedCategory as Category)
  );
  const legacyCategory = !activeFilter && categories.includes(requestedCategory as Category)
  ...
  const filtered = activeFilter
    ? availableThreads.filter((item) => (activeFilter.categories as readonly Category[]).includes(item.category))
    : legacyCategory
      ? availableThreads.filter((item) => item.category === legacyCategory)
    : availableThreads;
  return (
    <div className="page inner-page">
      ...
      <CategoryGrid activeCategory={activeFilter?.slug} />
      {loadError && <p className="data-status error" role="alert">{loadError}</p>}
      <div className="results-heading category-results"><h2>{activeFilter?.slug || legacyCategory || "Across Debatica"}</h2><span>{filtered.length} {filtered.length === 1 ? "thread" : "threads"}</span></div>
